...
LeggedMujocoHW::LeggedMujocoHW(mjModel *model, mjData *data, ::mujoco::Simulate & sim) 
...
bool LeggedMujocoHW::init(ros::NodeHandle& root_nh, ros::NodeHandle& robot_hw_nh){
  ...
  // it is used in ocs2::legged_robot::FromTopicStateEstimate
...
bool LeggedMujocoHW::setupJoints(){
  // check if the model is still valid
  if (mj_model_ == nullptr || mj_data_ == nullptr) {
    ROS_ERROR("[LeggedMujocoHW] Model or data is not valid");
    return false;
  }
...
void LeggedMujocoHW::read(const ros::Time& time, const ros::Duration& period) {
  // check if the model is still valid
  if (mj_model_ == nullptr || mj_data_ == nullptr) {
  ...
  // notice that we called LeggedMujocoHW::read first and then LeggedMujocoHW::write
  for(size_t i=0; i<jointNum_; i++){
...
void LeggedMujocoHW::write(const ros::Time& time, const ros::Duration& period) {
  // check if the model is still valid
  if (mj_model_ == nullptr || mj_data_ == nullptr) {
    ROS_ERROR("Model or data is not valid");
    return;
